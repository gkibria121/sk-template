...
class TestGetValues(unittest.TestCase):
    ...
    def test_object_variable(self):
        ...
        declarations = "$x = 'hi';$y = 'there';$z = [$x,$y] ;$a = [$z];"
        expected_result = {'$x': 'hi', '$y': 'there', '$z': ['hi','there'],'$a':[['hi','there']]}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    ...
    def test_single_variable(self):
        declarations = "$x = 42;"
        expected_result = {'$x': 42}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    def test_multiple_variables(self):
        declarations = "$x = 10;$y = 20;$z = $x + $y;"
        expected_result = {'$x': 10, '$y': 20, '$z': 30}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    def test_nested_variables(self):
        declarations = "$x = 5;$y = $x + 10;$z = $y * 2;"
        expected_result = {'$x': 5, '$y': 15, '$z': 30}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    ...
    def test_boolean_variables(self):
        declarations = "$x = True;$y = False;$z = $x and $y;"
        expected_result = {'$x': True, '$y': False, '$z': False}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    def test_complex_expression(self):
        declarations = "$x = 10;$y = 5;$z = ($x + $y) * 2 - $x;"
        expected_result = {'$x': 10, '$y': 5, '$z': 20}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    def test_array_with_expressions(self):
        declarations = "$x = 3;$y = 5;$z = [$x, $x + $y, $y * 2];"
        expected_result = {'$x': 3, '$y': 5, '$z': [3, 8, 10]}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
    def test_complex_array(self):
        declarations = "$x = 'hello';$y = 'world';$z = [['a', 'b'], [$x, $y], [1, 2, 3]];"
        expected_result = {'$x': 'hello', '$y': 'world', '$z': [['a', 'b'], ['hello', 'world'], [1, 2, 3]]}
        result = self.variable.get_result(declarations)
        self.assertEqual(result, expected_result)
